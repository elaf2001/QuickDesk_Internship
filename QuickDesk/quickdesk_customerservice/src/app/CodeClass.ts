...
class CounterManagement{
    ...
    takeNumber():number{
        ...
        } else if(this.last_serving !== 0 && this.queue.size() == 0 ){
    ...
    retriveFromStorage(){
        ...
        var raw_data = localStorage.getItem("COUNTER_MANAGEMENT"); 
        if(raw_data){
            var parsed_data = JSON.parse(raw_data); 
            
            //assign values 
            this.now_serving = parseInt(parsed_data.now_serving); 
            this.last_serving = parseInt(parsed_data.last_serving); 
            this.queue = new Queue(); 
            
            //getting the queue 
            var queue_list_json = JSON.parse(parsed_data.queue);
            this.queue = new Queue();
            this.queue.list = queue_list_json.list.map(function(item: string) {
                return parseInt(item, 10);
            });

            //counter list 
            var counter_list_json = JSON.parse(parsed_data.counter_list); 
            var new_counter_list = [];
            for (let i= 0; i<4; i++){
                
                var counter = new Counter(parseInt(counter_list_json[i]["counter_num"]));
                counter.setCurrServ(parseInt(counter_list_json[i]["curr_serving"])); 
                counter.setStatus(counter_list_json[i]["status"]); 
                new_counter_list.push(counter);
                this.counter_list = new_counter_list;
            }
    ...
    saveInStorage(){
        var data = {
            now_serving: this.now_serving.toString(),
            last_serving: this.last_serving.toString(),
            queue: JSON.stringify(this.queue),
            counter_list:JSON.stringify(this.counter_list)
        }

        localStorage.setItem("COUNTER_MANAGEMENT", JSON.stringify(data) ); 
    }
